...
public class TopologicalOrder extends GraphAlgorithm  {
	...
	List<Graph.Vertex> toplogicalOrder3(Graph g, List<Graph.Vertex> slist) {
		this.topNum = g.size();
		this.time = 0;
		this.cno = 0;
		...
		for (Graph.Vertex v : g
			...
			((cs6301.g40.TopologicalOrder.TopVertex) node[v.getName()]).cno = 0 ;
			((cs6301.g40.TopologicalOrder.TopVertex) node[v.getName()]).seen = false;
			((cs6301.g40.TopologicalOrder.TopVertex) node[v.getName()]).dis = 0;
		}
		
		while (!slist.isEmpty()) {
			Graph.Vertex u = slist.remove(0);
			if (!((TopologicalOrder.TopVertex) node[u.getName()]).seen) {
				this.cno++;
				list = DFSVisitRev(u, list);
				
			}
			
		}
		List<Graph.Vertex> slist1 = list;
		return slist1;
	}
	
	LinkedList<Graph.Vertex> DFSVisitRev(Graph.Vertex u, LinkedList<Graph.Vertex> list ){
